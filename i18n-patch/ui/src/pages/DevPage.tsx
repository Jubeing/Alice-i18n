...
const TABS: { key: Tab; labelKey: 'connectors' | 'tools' | 'sessions' }[] = [
  { key: 'connectors', labelKey: 'connectors' },
...
function SendSection() {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`${t.dev.sentTo} ${res.channel}:${res.to}`)
    } catch (err) {
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={t.dev.testMessagePlaceholder}
            rows={3}
    ...
    <div className="flex flex-1 min-h-0">
          ...
          {[...grouped.entries()].map(([group, tools]) => (
            <div key={group} className="mb-1">
              ...
              >
                ...
                <span className="font-semibold uppercase tracking-wider">{(t.tools as Record<string, string>)[group] ?? group}</span>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-5 py-4">
        {!selected ? (
          ...
          <p className="text-sm text-text-muted">{t.dev.loadFailed}</p>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-text-muted mt-1">{(t.toolDescriptions as Record<string, string>)[detail.name] ?? detail.description}</p>
      </div>

      {/* Input form */}
      {properties.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-xs font-semibold text-text-muted uppercase tracking-wider">{t.dev.input}</h3>
          {properties.map((prop) => (
            <div key={prop.key}>
              <label className="flex items-center gap-1.5 text-[13px] text-text mb-1">
                ...
                {prop.required && <span className="text-[10px] text-accent/70">{t.dev.required}</span>}
              </label>
              {prop.type === 'boolean' ? (
                <select
                ...
                <input
                  ...
                  value={inputs[prop.key] ?? ''}
                  onChange={(e) => setInputs((prev) => ({ ...prev, [prop.key]: e.target.value }))}
                  placeholder={((t.toolDescriptions as Record<string, string>)[prop.description] ?? prop.description) || prop.key}
              ...
              {prop.description && (
                <p className="text-[11px] text-text-muted/60 mt-0.5">{(t.toolDescriptions as Record<string, string>)[prop.description] ?? prop.description}</p>
              )}
            </div>
          ))}
        </div>
      ...
      <button
        ...
        className="btn-primary-sm"
      >
        {executing ? t.dev.executing : t.dev.execute}
      </button>
        ...
        <div className="mt-4">
          <div className="flex items-center gap-2 mb-2">
            <span className={`text-xs font-semibold ${result.data.isError ? 'text-red-400' : 'text-green-400'}`}>
              {result.data.isError ? t.dev.resultError : t.dev.resultOk}
            </span>
