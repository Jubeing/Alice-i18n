...
const ALL_PROVIDERS_KEYS = [
  { key: 'fmp', nameKey: 'fmp', descKey: 'fmpDesc', hintKey: 'fmpHint' },
  { key: 'fred', nameKey: 'fred', descKey: 'fredDesc', hintKey: 'fredHint' },
  { key: 'bls', nameKey: 'bls', descKey: 'blsDesc', hintKey: 'blsHint' },
  { key: 'eia', nameKey: 'eia', descKey: 'eiaDesc', hintKey: 'eiaHint' },
  { key: 'econdb', nameKey: 'econdb', descKey: 'econdbDesc', hintKey: 'econdbHint' },
  { key: 'intrinio', nameKey: 'intrinio', descKey: 'intrinioDesc', hintKey: 'intrinioHint' },
  { key: 'nasdaq', nameKey: 'nasdaq', descKey: 'nasdaqDesc', hintKey: 'nasdaqHint' },
  { key: 'tradingeconomics', nameKey: 'tradingeconomics', descKey: 'tradingeconomicsDesc', hintKey: 'tradingeconomicsHint' },
] as const

const ALL_PROVIDERS_KEYS_EN = [
...
function AssetProvidersSection({
  ...
  return (
    <ConfigSection
      title={t.marketData.assetProviders}
      description={t.marketData.assetProvidersDesc}
    >
        ...
        {Object.entries(PROVIDER_OPTIONS).map(([asset, options]) => {
          ...
          return (
            <div key={asset} className="flex items-center gap-3">
              <span className="text-[13px] text-text w-24 shrink-0 font-medium">{(t.marketData as Record<string, string>)[ASSET_LABELS[asset]] ?? ASSET_LABELS_EN[asset]}</span>
              <select
              ...
              {selectedProvider === 'yfinance' && (
                <span className="text-[13px] text-text-muted/50 px-1">{t.marketData.free}</span>
              )}
            </div>
          )
        })}
...
function ApiKeysSection({
  ...
  const [localKeys, setLocalKeys] = useState<Record<string, string>>(() => {
    ...
    for (const p of ALL_PROVIDERS_KEYS) init[p.key] = providerKeys[p.key] || ''
    return init
  })
  ...
  return (
    <ConfigSection
      title={t.marketData.apiKeys}
      description={t.marketData.assetProvidersDesc}
    >
      <div className="space-y-4">
        {ALL_PROVIDERS_KEYS.map(({ key, nameKey, descKey, hintKey }) => {
          const name = (t.marketData as Record<string, string>)[nameKey] ?? ALL_PROVIDERS_KEYS_EN.find(p => p.key === key)!.name
          const desc = (t.marketData as Record<string, string>)[descKey] ?? ALL_PROVIDERS_KEYS_EN.find(p => p.key === key)!.desc
          const hint = (t.marketData as Record<string, string>)[hintKey] ?? ALL_PROVIDERS_KEYS_EN.find(p => p.key === key)!.hint
            ...
            <Field key={key} label={name} description={hint}>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder={t.marketData.notConfigured}
                />
                <TestButton
                  status={status}
                  disabled={!localKeys[key] || status === 'testing'}
                  onClick={() => testProvider(key)}
                  testLabel={t.marketData.test}
                  testingLabel={t.marketData.testing}
                  okLabel={t.marketData.connected}
                  failLabel={t.marketData.failed}
                />
              </div>
            </Field>
          )
...
function AdvancedSection({
  ...
  return (
    <div className="py-6 border-b border-border/60 last:border-b-0">
      ...
      >
        <h3 className="text-[14px] font-semibold text-text">{t.marketData.advanced}</h3>
        <span className="text-[11px] text-text-muted/50">{expanded ? '\u25BC' : '\u25B6'}</span>
      </button>
      {!expanded && (
        <p className="text-[13px] text-text-muted/70">{t.marketData.dataBackendDesc}</p>
      )}
      {expanded && (
        <div className="space-y-6 mt-4">
          {/* Data Backend */}
          <div>
            <p className="text-[13px] font-medium text-text mb-2">{t.marketData.dataBackend}</p>
              ...
              <div className="mt-3">
                <Field label={t.marketData.apiUrl}>
                  <input
                    ...
                    placeholder={t.marketData.apiUrlPlaceholder}
                  />
                </Field>
              </div>
            )}
          ...
          {/* Embedded API Server */}
          <div>
            <p className="text-[13px] font-medium text-text mb-2">{t.marketData.embeddedApiServer}</p>
            <p className="text-[12px] text-text-muted/70 mb-3">
              {t.marketData.embeddedApiServerDesc}
            </p>
